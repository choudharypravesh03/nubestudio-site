...
function setTransform(e, t) {
    e.style.transform = t,
    e.style.willChange = t,
    e.style.WebkitTransform = t
}
window.addEventListener("load", (function() {
    performance.getEntriesByType("navigation");
    preloader && (sessionStorage.isVisited ? (document.querySelector("body").style.overflow = "",
    document.querySelector(".preloadContainer").style.opacity = 1,
    document.querySelector(".preloadContainer").classList.remove("preload"),
    document.querySelector(".home__intro__video video").play()) : (window.innerWidth <= 1024 && (preloader.style.height = Math.min(document.documentElement.clientHeight, window.screen.height, window.innerHeight) + "px"),
...
function startAnimation() {
    rafActive || (rafActive = !0,
    rafId = requestAnimationFrame(updateAnimation))
}
function updateAnimation() {
    var e = target - current
      , t = Math.abs(e) < .1 ? 0 : e * ease;
    return t ? (current += t,
    current = parseFloat(current.toFixed(2)),
    rafId = requestAnimationFrame(updateAnimation)) : (current = target,
    rafActive = !1,
...
function animateProjectsSlider() {
    ...
    if (e.hasClass("projects__slider")) {
        var r = current
          ...
          , a = .02 + n
          , d = .96 - n;
        if ($(".projects__slider__overlay .scrollbar__fill").css({
            transform: "scaleX(" + a + ")"
        }),
        $(".projects__slider__overlay .scrollbar__empty").css({
            transform: "scaleX(" + d + ")"
        }),
        changeTitle(c),
        t.each((function(e, r) {
            var i = $(this).find("img")
              ...
              , d = -500 * (s + a)
              , u = Math.abs((s + a) / 3);
            if ($(r).removeClass("isSelected"),
            $(t).eq(c).addClass("isSelected"),
            !(c >= o - 1)) {
                u = Math.max(1, Math.min(1.5, u + 1));
                var m = .75 * (s + a);
                m < 0 && (m = -m),
                i.css({
                    transform: "translateX( " + d + "px) scale(" + u + ")"
                }),
                n.css({
                    opacity: "" + m
                })
...
function animateHomeProjects() {
    ...
    if ((r = $("#projects")).length > 0 && window.innerWidth >= 768) {
        ...
        var u = current
          ...
          , h = .02 + (_ = Math.min(Math.max(_, 0), 1))
          , y = .96 - _;
        l.css({
            transform: "scaleX(" + h + ")"
        }),
        ...
        $(".selected-projects__slider").css({
            transform: "translateX(-" + p + "px)"
        });
        const w = m[s = (s = p / $(window).width()) > Math.ceil(s) - .25 ? Math.ceil(s) : Math.floor(s)]
          , S = m.find("img")[s]
        ...
        let k = j / 100;
        j /= 200,
        j = Math.max(1, Math.min(1.4, j + 1)),
        e = L,
        t = 1,
        o = 1.4,
        L = Math.min(Math.max(e, t), o),
        q.style.opacity = "" + k,
        S.style.transform = `scale(${j})`;
        var f = m.eq(s).data("title")
          , v = m.eq(s).data("subtitle");
        s != m.length - 1 && (m[s + 1].querySelector("img").style.transform = `scale(${L})`),
        previousScroll < u ? p / $(window).width() > Math.ceil(p / $(window).width()) - .25 && j >= 1 ? (n.text(f),
        c.text(v),
        ...
        g.querySelector(".currentProject").style.animation = "fadeIn 0.8s ease-out backwards") : p / $(window).width() > Math.ceil(p / $(window).width()) - .28 && j >= 1 && (n[0].style.animation = "fadeOut 0.8s ease-in both ",
        ...
        n.text(f),
        c.text(v),
        g.querySelector(".currentProject").innerText = s + 1) : p / $(window).width() < Math.ceil(p / $(window).width()) - .28 && j >= 1 ? (n.text(f),
        c.text(v),
        n[0].style.animation = "fadeIn 0.8s ease-out backwards",
        c[0].style.animation = "fadeIn 0.8s ease-out backwards",
        g.querySelector(".currentProject").innerText = s + 1,
        g.querySelector(".currentProject").style.animation = "fadeIn 0.8s ease-out backwards") : p / $(window).width() < Math.ceil(p / $(window).width()) - .25 && j >= 1 && (n[0].style.animation = "fadeOut 0.8s ease-in both",
        c[0].style.animation = "fadeOut 0.8s 0.1s ease-in both",
...
function updateAnimationHomeHero() {
    ...
    if (e) {
        ...
        let a, d, u = 1 - current / (l / 10);
        u = Math.min(Math.max(u, 0), 1),
        a = Math.min(Math.max(-10 * (1 - u), -10), 0),
        d = Math.min(Math.max(.2 * -current, -20), 0),
        ...
        p.innerHTML = _ < .5 ? s.dataset.start : _ >= 1 ? s.dataset.finish : s.dataset.halfway;
        let f = (current - e.getClientRects()[0].bottom) / (e.clientHeight - window.innerHeight);
        f = 1 - f,
        f = Math.min(Math.max(f, 0), 1),
        e.querySelector(".topGradient").style.opacity = .4 * f,
        s.style.opacity = f,
        setTransform(n, "scaleX(" + h + ")"),
        setTransform(c, "scaleX(" + y + ")")
    }
}
  ...
  , animateCtaHome = ()=>{
    ...
    document.querySelectorAll("#cta").forEach(o=>{
        if (window.innerWidth >= 768) {
            ...
            var d = .6 * (o.offsetTop - c);
            const u = isInside(e, document.querySelector(".cta__bottom"), d);
            setTransform(e, "translateY(-" + d + "px)"),
            u && $(".cta__links").addClass("anime"),
            t > current ? isInViewport(document.querySelector(".cta__bottom"), document.querySelector(".cta__bottom").getBoundingClientRect().bottom + 400) || $(".cta__links").removeClass("anime") : isInViewport(document.querySelector(".cta__bottom"), document.querySelector(".cta__bottom").getBoundingClientRect().bottom) || $(".cta__links").removeClass("anime")
        } else
            new Flickity(e,{
                cellAlign: "left",
                freeScroll: !0,
...
let newScrollPosition, oldScrollPosition = 0;
const goNextSlide = ()=>{
    var e = $(window).scrollTop() / $(window).height() + .01
      , t = document.querySelectorAll(".projects__item").length
      , o = Math.ceil(e) * $(window).height();
    Math.ceil(e) >= document.querySelectorAll(".projects__item").length || o <= $(window).height() * t && $("html, body").stop().animate({
        scrollTop: o
    }, 500, "swing")
}
  , goPrevSlide = ()=>{
    ...
    Math.round(e) - 1 < 0 || $("html, body").stop().animate({
        scrollTop: t
    }, 500, "swing")
}
  , headerAnimation = ()=>{
    ...
    const e = document.querySelector(".home")
      ...
      , s = ()=>{
        ...
        header.classList.remove("isBlur")
    }
      , n = ()=>{
        header.classList.remove("hidden"),
        header.classList.add("isBlur")
    }
      , c = ()=>{
        header.classList.remove("hidden"),
        header.classList.remove("isBlur"),
    ...
    ;
    e && (r() && !isInViewport(intro, 500) ? s() : i() && !isInViewport(intro, 0) ? n() : i() && isInViewport(intro, 0) ? (header.classList.remove("isBlur"),
    header.classList.add("noTransition")) : c()),
    ...
    document.querySelector(".drawer--active") && (header.classList.remove("hidden"),
    header.classList.remove("isBlack"),
    header.classList.remove("isDark")),
  ...
  , addBlackBGHeader = ()=>{
    ...
    for (let i = 0; i < e.length; i++)
        isInViewport(r = e[i], 0) && window.pageYOffset + r.getBoundingClientRect().top <= window.pageYOffset && (t(e[i]) ? (header.classList.add("isBlack"),
        header.classList.remove("isDark")) : o(e[i]) ? (header.classList.add("isDark"),
        header.classList.remove("isBlack")) : (header.classList.remove("isBlack"),
        header.classList.remove("isDark")));
    var r
}
  , isInViewport = (e,t)=>{
...
const toggleMenu = ()=>{
    ...
    document.querySelector("body").classList.contains("drawer--active") ? (headerClasses = e.getAttribute("class").split(" "),
    menuBtn.querySelector(".menubtn__title").innerHTML = "Close",
    e.classList.remove("isBlur"),
    e.classList.remove("hidden"),
    e.classList.remove("isBlack"),
    e.classList.remove("isDark")) : (menuBtn.querySelector(".menubtn__title").innerHTML = "Menu",
  ...
  , addRoute = e=>{
    ...
    if (document.querySelector("#projects-archive") && document.querySelector("#projects-archive").classList.contains("projects__slider")) {
        ...
        document.querySelector("#projects-archive") && (document.querySelector("#body").classList.add("clicked"),
        e.currentTarget.style.zIndex = "999",
        setTimeout(()=>{
            window.location = t
        }
        , 1100));
    t && !e.currentTarget.classList.contains("smooth-scroll") && (document.querySelector(".topbar").style.opacity = "0",
    document.querySelector("#body").classList.add("clicked"),
    e.currentTarget.style.zIndex = "999",
    setTimeout(()=>{
        window.location = t
  ...
  , moduleAnimation = e=>{
    isInViewport(e, 0) && (e.style.animation = "fadeIn 1.3s cubic-bezier(0.61, 0.01, 0.09, 1.01) backwards"),
    document.querySelector("footer").style.animation = "fadeIn 1.3s 0.5s cubic-bezier(0.61, 0.01, 0.09, 1.01) backwards"
}
  , effectElementsOnViewport = e=>{
    [...e.children].forEach(e=>{
        ...
        document.querySelector(".project__transform ") || document.querySelector(".services") || document.querySelector(".about") || [...e.children].forEach(e=>{
            moduleAnimation(e)
        }
        )
    }
...
const animeElementsOnViewport = ()=>{
    ...
    document.querySelector(".list__grid__item") && isInViewport(document.querySelector(".list__grid__item"), 0) && document.querySelectorAll(".list__grid__item").forEach(t=>{
        t.style.animation = "fadeIn  1.3s cubic-bezier(0.61, 0.01, 0.09, 1.01) forwards",
        t.style.animationDelay = e / 10 + "s",
        e++
    }
    )
}
...
const startTouch = e=>{
  ...
  , goSliderForward = ()=>{
    var e = document.querySelectorAll(".selected-projects__slide").length
      ...
      , s = 100 * (o + 1);
    o + 1 >= e || (document.querySelector(".selected-projects__slider").style.transform = `translateX(-${s}vw)`,
  ...
  , goSliderBack = ()=>{
    ...
    var e = [...document.querySelectorAll(".selected-projects__slide")].find(e=>e.classList.contains("isSelected"))
      ...
      , i = 100 * (t - 1);
    t - 1 < 0 || (document.querySelector(".selected-projects__slider").style.transform = `translateX(-${i}vw)`,
    document.querySelectorAll(".selected-projects__slide").forEach(e=>e.classList.remove("isSelected")),
  ...
  , moveTouch = e=>{
    ...
    let t = e.touches[0].clientX
      ...
      , r = initialX - t
      , i = initialY - o;
    Math.abs(r) > Math.abs(i) && (r > 0 ? goSliderForward() : goSliderBack()),
    initialX = null,
    initialY = null,
    e.preventDefault()
}
...
drawer.addEventListener("click", ()=>document.querySelector("body").classList.remove("drawer--active")),
...
document.querySelector(".selected-projects") && (document.querySelector(".selected-projects__arrow--next").addEventListener("click", goSliderForward),
document.querySelector(".selected-projects__arrow--prev").addEventListener("click", goSliderBack)),
...
document.querySelectorAll(".list__grid__item").forEach(e=>{
    e.addEventListener("mouseenter", t=>{
        (e.closest(".dropdown") || "A" == e.tagName) && (e.classList.add("animated"),
    ...
    e.addEventListener("mouseleave", t=>{
        (e.closest(".dropdown") || "A" == e.tagName) && e.addEventListener("animationiteration", ()=>{
            e.classList.remove("animated")
        }
        )
    }
...
jQuery((function(e) {
    ...
    function n() {
        ...
        o.removeClass("open"),
        o.find(".home__fullvideo__close").css("opacity", 0),
        i.pause()
    }
    ...
    e(".home__intro__video").each((function() {
        var t, r, i = e(".home__intro__btn"), c = e(".home__fullvideo__close");
    ...
    e("#projects").each((function() {
        var o = e(this)
          , r = (o.offset().top,
        o.find(".selected-projects__slider").children())
          , i = o.find(".title")
          , s = o.find(".subtitle")
          , n = (o.find(".scrollbar__fill"),
        o.find(".scrollbar__empty"),
        100 * r.length);
        const c = document.querySelector(".selected-projects__overlay");
        c.getBoundingClientRect().right;
        if (o.find(".selected-projects__slider").width(n + "vw"),
        ...
        window.innerWidth >= 768) {
            o.find(".selected-projects__content").height(n + "vh"),
            r[0].classList.remove("isSelected");
            o.outerHeight();
    ...
    e(window).scroll((function() {
        ...
        if (t <= 1) {
            var o = .02 + t
              , r = .96 - t;
            e(".projects .scrollbar--vertical .scrollbar__fill").css("transform", "scaleX(" + o + ")"),
            e(".projects .scrollbar--vertical .scrollbar__empty").css("transform", "scaleX(" + r + ")")
        }
    }
    ));
    const l = (e,t)=>{
    ...
    e(document).ready((function() {
        ...
        function _(t) {
            ...
            "grid" == t) {
                ...
                setTimeout(()=>{
                    ...
                    for (var l = 0; l < document.querySelectorAll(".projects__grid a").length - 1; l++)
                        document.querySelectorAll(".projects__grid a")[l].style.animationDelay = l / 10 + "s";
                    t.querySelectorAll(".projects__item__content").forEach(e=>{
                        e.style.backgroundColor = "rgba(255, 255, 255, 0.1)",
                        e.style.backdropFilter = "blur(20px)",
                        e.style.WebkitBackdropFilter = "blur(20px)"
                    }
                    ),
                    ...
                    i.each((function() {
                        e(this).on("change", (function() {
                            a.setFilterGroupSelectors(this.name, this.value),
                            a.parseFilterGroups()
                        }
                        )),
                        o && (a.setFilterGroupSelectors(this.name, r),
                        setTimeout((function() {
                            a.parseFilterGroups()
                        }
                        ), 500)),
                    ...
                    e(".projects__item").on("mouseover", (function() {
                        e(".projects__grid .projects__item .projects__item__image").css({
                            opacity: "0.4"
                        }),
                        e(this).children(".projects__item__image").css({
                            opacity: 1
                        })
                    }
                ...
                e("#projects-archive").find(".projects__item").each((function(t) {
                    var o = e(this).find("img")
                      , r = e(this).index()
                      , i = -500 * (r + 0)
                      , s = Math.abs((r + 0) / 3);
                    s = Math.max(1, Math.min(1.5, s + 1)),
                    o.css({
                        transform: "translateX( " + i + "px) scale(" + s + ")"
                    })
                }
                )),
        ...
        s.length && (document.location.search.includes("grid") || o ? _("grid") : _("slider"),
        window.innerWidth <= 1024 && _("grid"),
        document.querySelectorAll(".projects__item")[0].classList.add("isSelected")),
    ...
    e(window).on("scroll load", (function() {
        ...
        e(window).width() >= 768 && e(".service__item").each((function(o, r) {
            ...
            t > e(this).offset().top && t < s && (i.addClass("active").siblings().removeClass("active"),
    ...
    e(window).on("scroll", ()=>{
        let t = e(window).scrollTop() / a
          ...
          , r = .02 + o
          , i = .96 - o;
        e(".services .scrollbar__fill").css("transform", "scaleX(" + r + ")"),
        e(".services .scrollbar__empty").css("transform", "scaleX(" + i + ")");
        let s = 0;
        [...document.querySelectorAll(".workflow__bullet-lines")].forEach(e=>{
            isInViewport(e, 100) && (e.querySelector(".workflow__bullet-lines__outer-circle").style.transition = `stroke-dashoffset 0.6s ${s}s ease`,
    ...
    if (d && function(e) {
        ...
        s.forEach((function(e) {
            e.addEventListener("mouseenter", (function() {
                s.forEach((function(e) {
                    e.querySelector(".single-article__image").style.opacity = .6
                }
                )),
    ...
    let m, _ = e("#hero").outerHeight();
    e(window).scroll((function() {
        let t = e(window).scrollTop() / _
          , o = Math.min(Math.max(t, 0), 1)
          , r = .02 + o
          , i = .96 - o;
        e(".about .scrollbar__fill").css("transform", "scaleX(" + r + ")"),
        e(".about .scrollbar__empty").css("transform", "scaleX(" + i + ")")
    }
    ));
    const h = ()=>{
    ...
    e(".video__play").on("click", (function() {
        e(this).siblings(".video__full").addClass("open").fadeIn(300).children("video").get(0).play(),
        e(".topbar").css({
            opacity: "0",
            visibility: "hidden"
        }),
    ...
    e(".video__play").each((function() {
        var o = e(".video__btn");
        e("html").hasClass("no-touchevents") && (e(this).css("cursor", "none"),
        e(this).on("mouseover", (function() {
            o.css("opacity", 1)
        }
        )),
    ...
    e("#drop-down .list__grid__item").on("click", (function() {
        ...
        e(this).hasClass("list__grid__item--open") ? t.animate({
